...
const Dropdown = ({ productData, setProductData, name, title, label, options, disabled = false, formName }) => {
    ...
    return (
        <div className="select-wrapper w-full">
            <div className={`relative mb-5 ${disabled ? 'select-disabled text-disable' : ''}`}>
                <select
                    value={selectedValue}
                ...
                <label
                    className={`absolute left-0 transition-all duration-200 ease-in-out transform origin-left ${selectedValue ? '-top-2 text-md text-primary' : 'bottom-0 text-lg'} ${disabled ? 'opacity-60' : ''}`}
                >
